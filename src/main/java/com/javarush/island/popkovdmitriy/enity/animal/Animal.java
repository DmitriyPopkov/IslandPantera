package com.javarush.island.popkovdmitriy.enity.animal;
import com.javarush.island.popkovdmitriy.enityfactory.EnityFactory;
...
public abstract class Animal {
    ...
    public void worker() {
        lock.lock();
        this.actualSatiety = this.actualSatiety - (this.maxSatiety * Settings.workerPercent) - 0.01;
        lock.unlock();
    }
                ...
                for (int j = 0; j < islandArrays.length; j++) {
                    if (islandArrays[i][j].listAnimal.contains(this)) {
                        ...
                        if (sizeAnimal >= this.countOnOneCell) {
                            islandArrays[newI][newJ].listAnimal.remove(sizeAnimal-this.countOnOneCell);
                            return;
                        }
    ...
    public void reproduce(Cell cell) {
        ...
        try {
            ...
            if (randomNum >= Settings.randomMating) {
                ...
                listAnimal.add(EnityFactory.giveBirthAnimal(simpleName));
            }
        } finally {
            lock.unlock();
        }
